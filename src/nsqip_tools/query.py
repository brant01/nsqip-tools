...
class NSQIPQuery:
    ...
    def __init__(self, parquet_path: str | Path):
        ...
        self._lf = self._load_all_parquet_files()
    ...
    def _align_and_concat_schemas(
        ...
        for col_name in all_columns:
            ...
            # Precedence: String > Float64 > Float32 > Int64 > Int32 > Boolean
            ...
            for col_name in sorted(all_columns):  # Sort for consistent column order
                ...
                if col_name in current_schema:
                    ...
                    select_expressions.append(
                        pl.lit(None, dtype=target_type).alias(col_name)
                    )
    ...
    def filter_by_year(self, years: int | list[int]) -> Self:
        ...
        self._lf = self._lf.filter(pl.col("OPERYR").is_in(year_strs))
        return self

    def filter_by_cpt(self, cpt_codes: str | list[str], use_any: bool = True) -> Self:
        """Filter by CPT codes.
        ...
        if ALL_CPT_CODES_FIELD in schema_names:
            ...
            if use_any:
                ...
                expr = (
                    pl.col(ALL_CPT_CODES_FIELD)
                    .list.eval(pl.element().is_in(cpt_codes))
                    .list.any()
                )
            else:
    ...
    def filter_by_diagnosis(
        self, diagnosis_codes: str | list[str], use_any: bool = True
    ) -> Self:
        ...
        if ALL_DIAGNOSIS_CODES_FIELD in schema_names:
            ...
            if use_any:
                expr = (
                    pl.col(ALL_DIAGNOSIS_CODES_FIELD)
                    .list.eval(pl.element().is_in(diagnosis_codes))
                    .list.any()
                )
            else:
                expr = pl.all_horizontal(
    ...
    def filter_by_age(
        ...
        """Filter data by patient age range (in years).

        Uses AGE_AS_INT column for filtering.
        ...
        Examples:
            >>> query.filter_by_age(min_age=65)
            >>> query.filter_by_age(min_age=18, max_age=65)
        """
        if min_age is not None:
            self._lf = self._lf.filter(pl.col("AGE_AS_INT") >= min_age)
        if max_age is not None:
            self._lf = self._lf.filter(pl.col("AGE_AS_INT") <= max_age)
        return self

    def filter_elective(self) -> Self:
        """Filter to only elective (non-emergency) cases.

        Filters where EMERGENT == "No".

        Returns:
            Self for method chaining.
        """
        self._lf = self._lf.filter(pl.col("EMERGENT") == "No")
        return self
    ...
    def filter_active_variables(self, year_threshold: int = 2015) -> Self:
        ...
        # Get a sample after the threshold to check which columns have values
        sample_df = (
            self._lf.filter(pl.col("OPERYR") >= year_threshold)
    ...
    def sample(self, n: int = 1000, seed: int | None = None) -> pl.DataFrame:
    ...
    def describe(self) -> dict:
        ...
        return {
            ...
            "columns": len(cols),
            "column_names": cols[:10] + (["..."] if len(cols) > 10 else []),
    ...
    def __repr__(self) -> str:
        ...
        try:
            ...
            return f"NSQIPQuery(path={self.parquet_path.name}, rows={row_count:,})"
        except Exception:
            return f"NSQIPQuery(path={self.parquet_path.name})"
